...
const DRIVERS = new Set(["load", "view", "scroll"]);
...
function readDriver(element) {
  const driver = readString(element, "motion-driver", null);
  if (DRIVERS.has(driver)) return driver;

  // Backward-compatible fallback for existing uses of data-motion-on-view.
  return readBoolean(element, "motion-on-view", false) ? "view" : "load";
}
...
export const selectedCharacterYRotation = {
  ...
  mount(element, { gsap, reducedMotion }) {
    ...
    const driver = readDriver(element);
    const rotationY = readNumber(element, "motion-rotation-y", -360);
    const duration = readNumber(element, "motion-duration", 1.6);
    const stagger = readNumber(element, "motion-stagger", 0.16);
    const ease = readString(
      element,
      "motion-ease",
      driver === "scroll" ? "none" : "power2.inOut"
    );
    ...
    if (driver === "scroll") {
      ...
      const scrub = readNumber(element, "motion-scrub", 0.7);
      const scrollTrigger = resolveScrollContract(
        element,
        {
          trigger,
          start: readString(element, "motion-start", "top 82%"),
          end: readString(element, "motion-end", "top 55%"),
          scrub,
          invalidateOnRefresh: true
        },
        () =>
          viewportScroll(trigger, 0.82, () =>
            Math.max(window.innerHeight * 0.27, layoutSize(element).height * 2, 160)
          )
      );
      ...
      const timelineOptions =
        driver === "view"
          ? {
              scrollTrigger: resolveScrollContract(
                element,
                {
                ...
                () =>
                  viewportScroll(resolveTrigger(element), 0.88, () =>
                    Math.max(window.innerHeight * 0.3, layoutSize(element).height)
                  )
              )
            }
          : { delay: readNumber(element, "motion-delay", 0.15) };

      timeline = gsap.timeline(timelineOptions);
      timeline.fromTo(
        wrapped,
        { rotationY },
        {
