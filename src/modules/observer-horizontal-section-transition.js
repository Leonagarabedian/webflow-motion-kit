...
export const observerHorizontalSectionTransition = {
  name: "observer-horizontal-section-transition",
  category: "component",
  selector: '[data-motion~="observer-horizontal-section-transition"]',
  ...
  mount(element, { gsap, Observer, SplitText, reducedMotion }) {
    ...
    if (outerWrappers.some((node) => !node) || innerWrappers.some((node) => !node)) {
      console.warn(
        '[motion-kit] observer-horizontal-section-transition requires one [data-motion-observer-outer] and [data-motion-observer-inner] inside every panel.'
      );
      return;
    }
