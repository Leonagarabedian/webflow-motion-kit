...
export const heroFrameTransition = {
  ...
  start: "top top",
    ...
    { start: 0, end: closeEnd, travel: layoutSize(frame).width * closeTo },
    { start: shrinkStart, end: shrinkEnd, travel: layoutSize(frame).width * Math.abs(shrinkTo - closeTo) },
    { start: rotateStart, end: rotateEnd, travel: layoutSize(frame).height * Math.abs(finalRotation) / 180 },
    { start: squeezeStart, end: squeezeEnd, travel: layoutSize(frame).width * Math.abs(1 - shrinkTo) },
    { start: sideStart, end: sideEnd, travel: Math.max(...sides.map(side => layoutSize(side).width), 0) }
  ]))
})));
