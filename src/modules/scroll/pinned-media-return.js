import { scrollMode, layoutSize } from "../../core/scroll-alignment/contract.js";
import {
  readNumber,
  readString,
  selectTarget
  ...
  mount(element, { gsap }) {
    ...
    mm.add(
      ...
      ({ conditions }) => {
        ...
        const configuredVelocityForMax = readNumber(element, "motion-crop-velocity-max", 1400);
        ...
        const tick = (now) => {
          ...
          const velocityForMax = scrollMode(element) === "auto"
            ? Math.max(1, layoutSize(media).height || win.innerHeight) / Math.max(0.1, responseDuration)
            : configuredVelocityForMax;
