...
export const elementLayoutReturn = {
  ...
  mount(element, { ScrollTrigger, gsap, reducedMotion }) {
    ...
    if (syncTriggerId) {
      ...
      scrollTrigger = ScrollTrigger.create(resolveScrollContract(element, {
      ...
      }, () => (viewportScroll(trigger, 0.55, () => { const delta = getStableDelta(); return Math.max(window.innerHeight * 0.3, Math.hypot(axis === "y" ? 0 : delta.x, axis === "x" ? 0 : delta.y)); }))));
    }
