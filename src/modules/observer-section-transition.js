...
export const observerSectionTransition = {
  name: "observer-section-transition",
  category: "component",
  selector: '[data-motion~="observer-section-transition"]',
  ...
  mount(element, { gsap, Observer, SplitText, reducedMotion }) {
    ...
    if (outerWrappers.some((node) => !node) || innerWrappers.some((node) => !node)) {
      console.warn(
        '[motion-kit] observer-section-transition requires one [data-motion-observer-outer] and [data-motion-observer-inner] inside every panel.'
      );
      return;
    }
